...
# 20190109 Parameter runonce addiert
# 20190128 Catch mit Errorausgabe addiert
...
param(
	...
	[switch]$runonce = $false ,         # run only once
	...
	[string]$prtgpushurl)  # HTTP-PUSH-url
{
	if ($prtgpushurl -ne "" ) {
		...
		$Answer=Invoke-RestMethod `
			...
			write-Warning "SendTo-PRTG:Catch Request with Error `r`n $_.Exception.Message"
		}
...
try{
	...
	write-Host "End2End-EWS:USe ServiceURI: $($service.Url.AbsoluteUri)"
}
Catch {
...
try {
	...
	write-host "Unable to connect to Inbox with Error `r`n $_.Exception.Message"
	$inbox=$null
}
