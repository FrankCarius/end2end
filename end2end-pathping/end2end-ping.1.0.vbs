...
'
' Beschreibung
' Pings eine angegeben Adresse an und misst die Antwortzeit. Abweichungen von mehr als 5fache des Durchschnitts werden zu einer Eventlog Meldung
'
' Vorgehensweise
'
' Voraussetzung
...
'	Erste Version abgeleitet von end2end-ping
'-------------------------------------------------------------------------

' Konfigurationskonstanzen
const conIdleTime = 1000	' Ruhezeit zwischen zwei Ping
const Alarmdelta = 500	'Maximale abweichung des Messwerts vom mittelwert in Prozent
...
do
	...
	if mittelwert = 0 then 
		...
		if performance - mittelwert > Alarmdelta then
			message = "End2End PING ALARM: TTL exceeded limit" & vbcrlf &_
					vbtab & "Mittelwert: " & vbtab & mittelwert & "ms" & vbcrlf &_
					vbtab & "Aktueller Wert: " & vbtab & performance &  "ms" & vbcrlf &_
			...
			objDebug.writeln "end2end-ping: TTL:"&performance & vbtab & "Mittelwert=" & mittelwert ,3
		end if
	end if
	...
	if count > 9 then 
		objDebug.writeln " Mittel:" & cint(mittelwert) & "ms  Max:" & Max & "ms ",3
		count = 0 : max = 0 
	end if 
	count = count  + 1
