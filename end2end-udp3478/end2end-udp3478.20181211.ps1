...
# 20181205 Erweiterung um Analyse der Rückantwort
...
if ($remoteip -eq "o365") {
	write-host "  Using Skype for Business Online Servers"
	[string]$remoteip = "13.107.8.2"   # Anycast IP of Office 365 TURN Servers
}
elseif ($remoteip -eq "teams") {
	write-host "  Using Office 365 Microsoft Teams Server"
	[string]$remoteip = "52.113.193.5"   # Anycast IP of a teams Turn Server
}
else {
	write-host "  Using given IP-Addresse"
}
Write-host " TURN-Server to connect to: $($remoteip)"
		...
		try {
			...
			if ($receiveddata){
				...
				if ($ASCIIEncoder.GetString($receiveddata).Contains("The request did not contain a Message-Integrity attribute")) {
					...
					write-host " Invalid" -ForegroundColor yellow -nonewline
					$summary.failedrttcount++
				}
			}
			else {
				Write-host "No Answer Received 1" -foregroundcolor yellow
				$summary.failedrttcount++
			}
		}
		catch {
