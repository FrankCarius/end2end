...
param (
	...
	[long]$sleeptime = 50,			# time in seconds to sleep between two checks
	[string]$archivcsv = ".\End2End-udp3478_$($env:COMPUTERNAME).csv",   # store results
	[string]$prtgpushurl = "http://prtg:5050/end2end-udp3478	",
	[switch]$prtg=$false
)
...
Write-host " Connect UDPClient to $($remoteip):$($remoteudpport)"
...
while ($continue) {
	...
	if ($prtgpushurl -ne "") {
		...
		write-host "end2end-udp3478: Post Result to PRTGProbe $prtgpushurl"
