...
Mailboxpath(0) = "srv01" & vbtab & "Admin@msxfaq.local" & vbtab & "\\Top of Information Store\Kalender"
Mailboxpath(1) = "srv01" & vbtab & "user1@msxfaq.local" & vbtab & "\\Top of Information Store\Kalender"
...
do
	for intcount = lbound (Mailboxpath) to ubound (Mailboxpath)
		...
		if mapihelper.status <> "OK" then
			objDebug.writeln "Error opening mailbox:" & mapihelper.status
			csvfile.AddField "ERROR:" & mapihelper.status
		else
			dtopen  = now - dtstart
			objDebug.writeln "timeopen:" & dtopen 
			csvfile.AddField "timeopen" & dtopen 
			
			objFolder = mapihelper.OpenFolder(objMailbox, arrMBEntry(2)) 
			if mapihelper.status <> "OK" then
				objDebug.writeln "Error opening folder:" & mapihelper.status
				csvfile.AddField "ERROR:" & mapihelper.status
			else
				dtfolder  = now - dtstart
				objDebug.writeln "timefolder:" & dtfolder
				...
				next
				dtitem  = now - dtstart
				objDebug.writeln "timeread:" & dtitem
